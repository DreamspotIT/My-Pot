...
<div class="card shadow-sm">
  <div class="card-header d-flex justify-content-between align-items-center">
    <h5 class="mb-0">{{ isset($gold_offer) ? 'Edit' : 'Add' }} Gold Offer</h5>
    ...
    <div class="card-body">
        <form method="POST" action="{{ isset($gold_offer) ? route('gold-offers.update', $gold_offer->id) : route('gold-offers.store') }}">
            ...
            <button type="submit" class="btn btn-success">{{ isset($gold_offer) ? 'Update' : 'Create' }}</button>
            <a href="{{ route('gold-offers.index') }}" class="btn btn-secondary">Back</a>
        </form>
    </div>
</div>
@endsection
